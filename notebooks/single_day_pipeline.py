...
# 0.5 时为 0.175，1.25 时为 0.119，1.5 时为 0.111，8.0 时只剩 0.035。
#
...
def granularity_report(
    markov_time, flow=None, sequences=None, size=CELL_SIZE_M, seed=RANDOM_SEED
):
    flow = cell_flow if flow is None else flow
    sequences = cell_sequences if sequences is None else sequences
    assignment, steps = postprocess(
        infomap_partition(flow, markov_time, seed=seed), flow
    )
...
# %%
# 1.25 与 1.5 在上表里的差距只有 2–3%，必须确认它不是 Infomap 单个随机种子的产物。
# 换四个种子把三个候选粒度各重跑一遍，看区域数与非对角线指标的排序是否稳定。
SEED_CHECK_SEEDS = [42, 7, 2020, 1234]
SEED_CHECK_TIMES = [1.0, 1.25, 1.5]

seed_check = pd.DataFrame(
    [
        granularity_report(markov_time, seed=seed) | {"seed": seed}
        for seed in SEED_CHECK_SEEDS
        for markov_time in SEED_CHECK_TIMES
    ]
)[["seed", "markov_time", "regions", "od_self_loop_share", "channel_total"]]
display(
    seed_check.pivot(
        index="seed",
        columns="markov_time",
        values=["regions", "od_self_loop_share", "channel_total"],
    ).round(3)
)
best_regions = seed_check.loc[seed_check.groupby("seed")["regions"].idxmax()]
best_channel = seed_check.loc[seed_check.groupby("seed")["channel_total"].idxmax()]
print(
    f"seeds where 1.25 wins on regions: {int(best_regions['markov_time'].eq(1.25).sum())}"
    f" / {len(SEED_CHECK_SEEDS)}; "
    f"on channel flow: {int(best_channel['markov_time'].eq(1.25).sum())}"
    f" / {len(SEED_CHECK_SEEDS)}"
)

# %% [markdown]
# **结果**：粗的一侧结论清楚。区域变粗，订单自环比例单调上升（`markov_time = 6.0` 时 63.8% 的行程
# 不出区域，非对角线基本被抽空），跨 ≥2 区域的轨迹比例与通道流总量同步塌缩（2.0 起 476 对 / 13,218，
# 6.0 只剩 142 对 / 5,684）。2.0 以粗不可用。
#
# 细的一侧却不是「越细越好」，而是一段平台。0.75–1.75 这一整段里，区域中位面积落在
# 0.472–0.551 km²（中位宽度 687–742 米），只有后处理面积下限 `MIN_COMPONENT_CELLS = 14`
# （0.315 km²）的 1.5–1.75 倍；区域数也只在 128–141 之间浮动。段内改变 `markov_time`，
# 改变的主要是原始社区的碎裂程度（470 → 181）和被后处理并掉的连通块数（341 → 53），
# 而不是最终的区域尺度——**区域尺度是后处理的面积下限定的，不是 `markov_time` 定的**。
#
# 因此段内选点不能看尺度：区域中位宽度几乎不动，区域数也不动、且对 `markov_time` 不单调
# （0.75 得 129，反而少于 1.0 的 138 和 1.25 的 141）。两个量都不构成判据。
# 跨格边长比较时（5.6）仍应看区域中位宽度而不是区域数——那里区域数因栅格化不同而不可比，
# 宽度才是同量纲的——但在同一格边长的段内，它们都只反映那条面积下限。
#
# 段内真正有分辨力的是非对角线质量：订单自环比例在 1.25 取全扫描的最小值 30.5%，
# 跨区订单 34,293 单、跨 ≥2 区域轨迹 63.0%、通道对 590、通道流 15,229 同时取最大值。
# 但这五列不是五条独立证据——每区域通道流在整段几乎是常数
# （110.8 / 107.3 / 108.0 / 108.6 / 114.9），五个指标基本都随区域数走，
# 它们合起来说的是同一件事：1.25 在这段平台里给出最多的区域，因而非对角线上的量最足。
#
# 这个差只有 2–3%，所以上面那格换了四个 Infomap 种子重跑三个候选粒度。排序是稳的：
# 1.25 在 4 个种子上都给出最多的区域（146 / 141 / 145 / 139，对 1.5 的 139 / 137 / 136 / 136）
# 与最低的订单自环比例（29.4% / 30.5% / 29.6% / 30.6%，对 1.5 的 30.4% / 31.6% / 31.3% / 31.4%）；
# 通道流赢 3 个种子，种子 7 上 1.5 以 15,515 对 15,421 反超——而那里 1.5 的区域数反而更少，
# 说明通道流并不严格由区域数决定，只是大体同向。结论按最稳的两列（区域数、自环比例）取 1.25。
...
#
# - **区域（region）**：在 150 米单元格流网络上跑 `markov_time = 1.25`，用于 OD 矩阵、通道流矩阵、
#   过境率与全部画像指标；
# - **片区（district）**：把区域当节点、区域间通道流当链路，再跑一次 Infomap（`markov_time = 0.5`），
#   用于弦图、人工标签与前端的粗粒度视图。人工标签只做到片区这一级。
#
# 段内选 1.25 有两条理由，且都不依赖区域尺度——按 5.4，尺度在 0.75–1.75 整段上是由后处理的
# 面积下限钉死的，1.25 与 1.5 的区域中位宽度同为 719 米，本来就分不出高下：
#
# 1. **非对角线上的量最足**：1.25 在全扫描上同时取到订单自环比例的最小值与通道流的最大值
#    （30.5% / 15,229，对 1.5 的 31.6% / 14,879）。如 5.4 所述这只是一条证据而非五条，
#    因为这些指标都随区域数走，实质是 1.25 在平台内给出最多的区域（141 对 137）。
# 2. **`excess_ami` 更高**（0.119 对 0.111）。原始的 `split_half_ami` 反过来（0.758 对 0.764），
#    但 5.3 已经证明这个量系统性偏向粗粒度，判据取的是扣掉格网零模型之后的净值。
#
# 两条的差距都不大（通道流 2.4%、`excess_ami` 0.008），选点本身是薄的，5.4 的换种子对照就是为了
# 确认这个薄差不是单个 Infomap 种子的产物。5.6 的基线行**不**构成第三条理由：那里的目标区域数
# 取自采纳划分本身，无论采纳哪个 `markov_time`，150 米那一行都会落回采纳划分，
# 那是 `TARGET_REGIONS` 定义的性质，不是选点的证据。
#
# 代价要讲清楚：1.25 的后处理干预更重，267 个原始社区里并掉 126 个（47%），1.5 是 218 并掉 81 个
# （37%）。但这不是一项独立缺陷——被并掉的都是小于 14 格的连通块，并入口径确定（流量最强的邻接区域），
# 而区域尺度本来就由这条下限决定，并掉多少只是同一件事的另一面。
#
# 再细则越过平台：`markov_time = 1.0` 得 346 个原始社区、原始中位宽度 474 米，
# 后处理要并掉 208 个连通块才收回到同样的尺度，那是把区域本身切碎再拼回来，而不是把边界画细。
#
...
# **结果**：
#
# - 后处理四步：267 个原始社区 → 并掉 126 个小于 14 格的连通块 → 141 个区域，没有被单一区域包围的孤立格。
#   片区 35 个、中位面积 2.272 km²。两级划分的每一个单元都空间连通、且是单部件多边形。
...
# - 成图层覆盖率由分析层的 55.1%（80.6 km²）升到 78.7%（115.1 km²），孔洞由 33 个归零，
#   9 轮 BFS 收敛。未覆盖格的集合只由流网络决定、与 `markov_time` 无关，因此上面三条空地判定
#   在换粒度时不变；变的只是分析层被切出多少个孔洞。
...
#
# 1. **控制区域尺度而不是控制 `markov_time`**：每个格边长各自扫出使区域数最接近采纳划分
#    （`TARGET_REGIONS`）的 `markov_time`，再比较边界质量。取采纳划分的区域数而不是某个外来整数，
#    是为了让 150 米那一行按构造等于 5.5 采纳的划分、`ami_vs_adopted = 1.000` 成为恒等式，
#    另外两行才有稳定的参照系；
...
# 格边长敏感性：在区域尺度相同的前提下比较。共同工作点取采纳划分的区域数，
# 这样 150 米那一行按构造就是 5.5 采纳的划分本身、ami_vs_adopted 是恒等式，
# 另外两行才有一个稳定的参照系；取一个外来的整数则这条性质只能靠碰巧成立。
# 对每个格边长先扫出使区域数最接近该目标的 markov_time，再比较边界质量与稳定性。
# min_cells 同样按面积等价折算，不沿用固定格数。
TARGET_REGIONS = len(region_members)
...
# %% [markdown]
# **结果**：Leiden 在 `resolution = 8` 上得到 132 个社区（最接近采纳的 141 个区域），与 Infomap
# 区域的 AMI 为 0.805；AMI 在 `resolution = 4–16` 的整个区间都稳定在 0.79–0.81，
# 说明这套划分不是 Infomap 目标函数的产物。
#
# 格边长敏感性（对齐目标即采纳划分的 141 个区域，三种边长实得 141 / 151 / 145）：区域中位宽度
# 落在 719–794 米、订单自环比例 30.1%–30.5%，都很接近；差别集中在成图质量——分区层覆盖率随格子
# 变小而**下降**（300 米 74.0% → 200 米 64.5% → 150 米 57.3%），孔洞由 0 增到 10 再增到 33。
# 格子越小，轮廓越贴着路网，被顺带扫进来的空地越少，这正是 5.5 需要单独做一层成图几何的原因。
#
# 150 米那一行的 `ami_vs_adopted = 1.000` 是恒等式而不是结果：目标区域数取的就是采纳划分的区域数，
# 该行必然落回 `markov_time = 1.25`、必然是同一套划分。它的作用是给另外两行一个参照系——
# 200 米 0.764、300 米 0.752，是同一尺度上的三种栅格化，不是三个不同的分区结论。
...
# %% [markdown]
# **结果**：141 个区域全部有订单；落在无覆盖单元格、需要归到最近区域的订单占 2.12%（解锁）与 1.05%（上锁），
# 不影响源汇的量级。净流入最强的是环岛干道 26 号（+767）与 28 号（+700）、枋钟路 72 号（+630）、
# 环岛东路 25 号（+590）、火炬北路 8 号（+530）；净流出最强的是岐山北路 31 号（−535）、
# 嘉禾路 6 号（−507）、枋钟路 16 号（−426）、前埔东路 43 号（−404），早高峰的方向性清楚。
# 6 个区域没有电子围栏、16 个区域没有 landuse 覆盖，6.5 与 6.6 的样本量因此小于 141。
...
# 12.5% 的行程跨小时，这些上锁事件的归属小时因此改变，141 个区域中 32 个的峰值小时随之移位；
# 净流入型区域的到达被整体推后到 8–9 点（环岛干道 28 号 8 点 +517、9 点 +177，26 号 8 点 +513、
# 9 点 +180）。各区域的全天净流不变——分小时净流按行求和仍等于 6.2 的 `net_inflow`。
#
# 符号翻转：86 个区域在 6–10 点内出现过符号变化、32 个变化两次以上，但把幅度限定到单小时净流绝对值 ≥ 10 后
# 也只剩 32 个，全体区域的平均单小时净流绝对值为 35.37。单位面积净流最极端的十个区域里一次符号变化都没有，
# 四小时全程同号；差别只在峰值小时——三个落在 7 点（岐山北路 31、殿前一路 115、枋湖北二路 111）、
# 七个落在 8 点，净流出与净流入两侧都跨着这两个小时。
#
...
# %% [markdown]
# **结果**：只用完整落在 6:30–9:30 内的 13,087 条轨迹（占有效轨迹 88.7%）重算，110 个可比区域的 `PI_r`
# 与全窗口的 Spearman 为 0.981、中位数 0.201 → 0.196，排序基本不受 6:00/10:00 硬截断影响。
#
# 过境率最高的三个区域（思明南路 86、云顶北路 138、吕岭路 128）`R` 普遍偏低（0.10–0.27），
# 按 180 度折叠的 `R_axial` 也只有 0.14–0.24，说明这些区域的过境方向并不集中在单一朝向，
...
# %% [markdown]
# **结果**：净流入强度与「就业 + 教育占比」只有弱正相关——面积构成 Pearson 0.132（n=125）、Spearman 0.071；
# 点位构成 Pearson 0.207（n=97）、Spearman 0.187。方向与预期一致，但强度不足以支撑
...
# %% [markdown]
# **结果**：失衡的一端是「净流入高、泊位供给低」——枋钟路 72 号（净流入 1,217 /km²、围栏面积
# 634 m²/km²）、环岛干道 28 号（943 / 354）、环岛干道 26 号（1,003 / 2,950）；
# 另一端是「净流出后泊位大量空置」——湖滨东路 49 号（净流出 364 /km²、围栏 5,002 m²/km²）、
# 吕岭路 53 号（243 / 5,457）、嘉禾路 6 号（469 / 3,752）。
...
# %% [markdown]
# **结果**：支撑集差异已量化——通道流的 590 个区域对中 99.8% 相邻，出行流的 3,016 个区域对中只有
# 20.3% 相邻，两个矩阵确实不能逐格比较。通道流的相邻率不是恰好 100%：去抖会把不够格的一次进入
# 并进前一次，于是极少数相邻转移在区域序列上跨过了中间区域。
#
# 代表性检验：1,563 个共同区域对上 Spearman 0.826，这些区域对覆盖了 91.6% 的区间订单；
# 真正未被任何轨迹采到的相邻区域对只有 44 个、承载 176 条订单。轨迹侧的硬剔除没有把需求结构抽歪。
#
# 过境倍数：529 条可定义走廊的中位数 2.333，其中 314 条超过 2 倍。按穿越次数排序的主走廊
# （吕岭路 103→42、环岛干道 56→环岛东路 25、吕岭路 42→环岛干道 28）倍数在 1.13–2.88；
# 倍数最高的一批（思明南路 86→厦禾路 7 为 18.7、湖滨北路 89→57 为 11.2、
# 湖滨南路 114→湖滨西路 122 为 11.0）端点需求只有个位数，是典型的纯过路走廊。
...
# %% [markdown]
# **结果**：9,617 条长度 ≥ 2 的序列，来自 9,302 条轨迹（占有效轨迹 63.0%），141 个区域全部出现过。
# 序列很短：中位长度 2、P90 为 4，长度 ≥ 5 的只有 502 条（5.2%），最长一条穿过 28 个区域。
#
# 阈值扫描显示计划的默认值 0.001 已经在可用区间的边缘。绝对门槛 15 条序列时共 498 条长度 ≥ 2 的模式，
# 其中长度 3 只剩 77 条、长度 4 只剩 1 条、长度 ≥ 5 归零；放宽一档到 0.0005（门槛 7）才有 327 / 35 / 1。
# 反向收紧到 0.005 时只剩 42 条区域对、连三元链都没有，0.02 时全部归零。
# 也就是说，要挖的是「通勤链」而不是「区域对」的话，支持度不能高于 0.001。
#
# 子序列与连续子序列的差别是实的：同一阈值下 PrefixSpan 得到 498 条模式，其中只有 387 条连续，
# 另外 111 条（22.3%）跳过了中间区域；长度 2 的 420 条模式里有 18.1% 的区域对在空间上并不相邻，
# 属于「两端都常经过、但中间怎么走不定」。做走廊归因应取连续子序列，
# PrefixSpan 的完整输出更接近「常见起讫组合」，两者不能混用。
#
# 高支持度的长链高度集中在同一条走廊：长度 3 的前十条与长度 4 唯一的一条都落在吕岭路一线
# （`12 金山路 → 42 吕岭路 → 28 环岛干道` 支持度 55，`128 → 9 → 103 → 42` 四段全在吕岭路上，支持度 20）。
# 长度 2 的第二名 `103 吕岭路 → 42 吕岭路`（212 条）与 6.7 通道流按穿越次数排出的榜首（212 次）
# 完全相等；榜首 `42 吕岭路 → 28 环岛干道`（223 条、2.32%）则高于其 199 次相邻穿越。
# 两者同源，通道流按轨迹去重计**相邻**转移，这里按序列条数计**含子序列**，因此后者只多不少。
...
# %% [markdown]
# **结果**：35 个片区全部拿到了道路名标签，没有需要保留编号的片区。片区中位面积 2.27 km²，
# 最大的三个是长岸路 / 成功大道（8.03 km²）、枋钟路 / 环岛干道（5.08 km²）、
# 东渡路 / 湖滨西路（4.07 km²）。
