...
| ④ 架构与业务功能完整性 | 25% | 四层链路完整；单地图 + 三种分析图层 + 地理聚焦；区域源汇/过境画像与显著流对识别 |
...
| 前端层 | Vue 3、TypeScript、Vite、Leaflet、ECharts | 单地图、三种分析图层、全局工具栏、地理聚焦、弦图与方向玫瑰 |
...
前端使用“单地图画布 + 三种分析图层 + 全局工具栏 + 地理聚焦状态”。全局工具栏控制内容图层、日期、连续整点范围和 `average/sum`；默认显示晴天集完整 `[06:00, 10:00)` 的跨日平均。可加量先累计所选时段，`average` 只除以日期数，`sum` 保留跨日合计。

- **源汇图层**：区域按净流入强度填色，以数值符号、文字和动态对称图例同时表达源、汇与零。点击区域进入地理聚焦，不在常规分析状态增加独立页面或常驻侧栏。
- **区域间流动图层**：可切换出行流/通道流及仅显著/全部流对。地图和列表共享稳定排序后的 Top 50 区域流；ECharts 弦图按片区聚合全部过滤后的非自环流，不受 Top 50 截断。
- **典型通勤链图层**：地图显示接口返回的区域序列并默认选择第一条，地图对象和有序列表共享选择。该图层使用日期作用域，不使用全局时段或 `average/sum`。
- **地理聚焦状态**：从任一图层点击区域或绘制合法矩形后，保留并压暗原地图、筛选、侧栏和对象选择，只查询有效轨迹。退出按钮或 Escape 清除局部查询，并原样恢复进入前的分析状态与地图视图。
...
- 前端完成单地图、三种分析图层、全局工具栏、地理聚焦、弦图、方向玫瑰与框选联机查询。
