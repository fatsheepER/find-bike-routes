...
因此段内选点不能用尺度类的量：区域中位宽度几乎不动，区域数也不动、且对 markov-time 不单调。段内的判据是两条——出行流矩阵非对角线上的量（订单自环比例取最小、通道流总量取最大，二者同源于「平台内区域数最多」，算一条证据）与 `excess_ami`。两条判据在相邻取值间的差距只有百分之几，因此选定前须换若干随机种子重跑候选粒度，确认排序不是单次社区检测的产物。跨格边长比较时仍看**区域中位宽度**而不是区域数：那里区域数因栅格化不同而不可比，宽度才同量纲。
...
另在 150/200/300 米格边长上做敏感性对比。对比须先把三种格边长各自扫到**相同的区域尺度**，否则比的是「区域变细」而不是「边界变细」；`min_cells` 一并按面积等价折算。对齐用的目标区域数取**采纳划分自身的区域数**，这样 150 米那一行按构造就等于采纳划分、与之的 AMI 是恒等式，另外两行才有稳定的参照系。
...
课程文档以仓库中的 SDD 为事实来源，Matt Pocock 工作流用于执行。开发顺序：更新并评审 SDD → `to-spec` 生成执行快照 → `to-tickets` 拆纵向切片 → `implement` 实现测试提交 → code review 并更新验证记录与提示词日志。
